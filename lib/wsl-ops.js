 ...
 */
/**
 * Build the bash script used to scan for stale directories.
 * Extracted as a helper for testability.
 * @param {string} nameTests - find -name tests (e.g. "-name node_modules -o -name vendor")
 * @param {number} staleDays - age threshold in days
 * @returns {string} bash script content
 */
function buildStaleScanScript(nameTests, staleDays) {
  return [
    '#!/bin/bash',
    '',
    '# Detect if git is available; if not, skip all git checks',
    'HAS_GIT=0',
    'command -v git >/dev/null 2>&1 && HAS_GIT=1',
    '',
    '# Cache for git repo root lookups (avoids redundant git rev-parse calls)',
    'declare -A GIT_ROOT_CACHE',
    '',
    '# Resolve git repo root for a directory, caching results.',
    '# Sets REPLY to repo root or "__none__".',
    'get_git_root() {',
    '  local dir="$1"',
    '  local check="$dir"',
    '  while [ "$check" != "/" ] && [ -n "$check" ]; do',
    '    if [ -n "${GIT_ROOT_CACHE[$check]+x}" ]; then',
    '      REPLY="${GIT_ROOT_CACHE[$check]}"',
    '      GIT_ROOT_CACHE["$dir"]="$REPLY"',
    '      return',
    '    fi',
    '    check=$(dirname "$check")',
    '  done',
    ...
    '    GIT_ROOT_CACHE["$(dirname "$dir")"]="$root"',
    '    REPLY="$root"',
    '  else',
    '    GIT_ROOT_CACHE["$dir"]="__none__"',
    '    GIT_ROOT_CACHE["$(dirname "$dir")"]="__none__"',
    '    REPLY="__none__"',
    '  fi',
    '}',
    '',
    `find /home /root /var /tmp /opt -maxdepth 8 -type d \\( ${nameTests} \\) -mtime +${staleDays} -prune -print 2>/dev/null | while IFS= read -r dir; do`,
    '  # If git is available, skip directories inside git repos that are NOT gitignored',
    '  if [ "$HAS_GIT" -eq 1 ]; then',
    '    get_git_root "$dir"',
    '    git_root="$REPLY"',
    '    if [ "$git_root" != "__none__" ]; then',
    '      if ! git -C "$git_root" check-ignore -q "$dir" 2>/dev/null; then',
    '        continue',
    '      fi',
    '    fi',
    '  fi',
...
function scanStaleDirs({ distro, days }) {
  ...
  return new Promise((resolve) => {
    ...
    const scriptContent = buildStaleScanScript(nameTests, staleDays);
