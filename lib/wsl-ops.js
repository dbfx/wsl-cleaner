...
function getHealthInfo(distro) {
  return new Promise((resolve) => {
    const scriptContent = [
      ...
      'echo "---PROCS---"',
      'ps -eo pid,user,%cpu,%mem,comm --no-headers --sort=-%cpu 2>/dev/null | head -20',
      'echo "---OSRELEASE---"',
      'cat /etc/os-release 2>/dev/null',
      'echo "---PORTS---"',
      'ss -tlnp 2>/dev/null',
      'echo "---DNS---"',
      'nslookup google.com 127.0.0.53 2>&1 | head -10; echo "EXIT=$?"',
      'echo "---IOPRESSURE---"',
      'cat /proc/pressure/io 2>/dev/null',
      'echo "---ZOMBIES---"',
      'ps -eo pid,user,stat,comm --no-headers 2>/dev/null | grep " Z"',
      'echo "---DOCKER---"',
      ...
      'echo "---PACKAGES---"',
      'dpkg -l 2>/dev/null | tail -n+6 | wc -l || rpm -qa 2>/dev/null | wc -l',
      'echo "---GPU---"',
      ...
      'echo "---INTEROP---"',
      'cat /proc/sys/fs/binfmt_misc/WSLInterop 2>/dev/null | head -1',
      'echo "---WSLCONFIG---"',
    ...
    proc.on('close', (code) => {
      ...
      if (code !== 0 && output.trim() === '') {
        resolve({ ok: false, error: 'Could not retrieve health data (exit code ' + code + ').' });
        return;
      }
...
function parseHealthOutput(raw) {
  ...
  // Processes
  const procLines = (sections.PROCS || []).filter(l => l.trim());
  const processes = [];
  ...
  return { kernel, uptime: { seconds: uptimeSeconds, formatted: uptimeFormatted }, memory, cpu, disk, network, processes };
}
